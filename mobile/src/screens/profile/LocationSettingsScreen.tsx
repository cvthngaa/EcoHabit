...
const LocationSettingsScreen: React.FC<any> = ({ navigation }) => {
 ...
 setSettings(stored.location);
 };
 fetchSettings();
 }, []);

 const toggleSetting = async (key: keyof typeof settings) => {
 if (key === 'preciseLocation' && !settings.preciseLocation) {
 const { status } = await Location.requestForegroundPermissionsAsync();
 if (status !== 'granted') {
 showToast('Vui lòng cấp quyền vị trí trong cài đặt thiết bị.', 'warning');
 return;
 }
 }
 ...
 await saveSettings({ location: settings });
 showToast('Đã cập nhật thiết lập vị trí.', 'success');
 };
