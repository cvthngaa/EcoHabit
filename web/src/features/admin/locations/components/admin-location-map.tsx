...
const AdminLocationMap: React.FC<AdminLocationMapProps> = ({ locations, onViewDetail }) => {
          ...
          >
            <Popup minWidth={220} maxWidth={260}>
              <div className="p-1 space-y-2">
                {/* Name + Status */}
                <div>
                  <p className="font-bold text-slate-900 text-sm leading-tight">{loc.name || 'Không tên'}</p>
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold mt-1 ${loc.status ? STATUS_COLOR[loc.status] : STATUS_COLOR['PENDING']}`}
                  >
                    {loc.status ? STATUS_LABEL[loc.status] : STATUS_LABEL['PENDING']}
                  </span>
                </div>

                {/* Partner Name */}
                {loc.partnerProfile?.organizationName && (
                ...
                <div className="text-[11px] text-slate-500">
                  ...
                  {loc.type ? TYPE_LABEL[loc.type] : 'N/A'}
                </div>
                ...
                {onViewDetail && (
                  <button
                    ...
                    className="w-full mt-1 py-1.5 text-[11px] font-semibold bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors cursor-pointer"
                  >
                    Xem chi tiết →
                  </button>
                )}
