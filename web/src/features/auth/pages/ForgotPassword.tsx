...
import { Mail, Lock, ArrowLeft, ArrowRight, Loader2, AlertCircle, CheckCircle2 } from 'lucide-react';
import { useForgotPasswordOtp } from '../services/use-forgot-password-otp';
import { useVerifyOtp } from '../services/use-verify-otp';
import { useResetPassword } from '../services/use-reset-password';
import { z } from 'zod';
...
export const ForgotPassword: React.FC = () => {
  ...
  // API mutations
  const sendOtpMutation = useForgotPasswordOtp();
  ...
  // Handle Step 2 Submit: Verify OTP and Reset Password
  const handleStep2Submit = (e: React.FormEvent) => {
    ...
    const validation = resetPasswordSchema.safeParse({ email, password, confirmPassword });
    ...
    verifyOtpMutation.mutate(
      ...
      {
        onSuccess: () => {
          // If OTP is correct, perform password reset
          resetPasswordMutation.mutate(
            { email, newPassword: password },
            {
              ...
              onError: (resetErr: any) => {
                const msg = resetErr?.response?.data?.message || 'Không thể đặt lại mật khẩu. Vui lòng thử lại.';
                setErrors({ general: msg });
              },
            }
          );
      ...
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-200/80 shadow-xl shadow-slate-100/40 p-8 space-y-6 transition-all">
        ...
        {step === 1 && (
          <>
            <div className="text-center space-y-2">
              ...
              <h2 className="text-2xl font-bold text-slate-900 tracking-tight mt-3">Quên Mật Khẩu</h2>
              <p className="text-sm text-slate-500">Nhập email doanh nghiệp của bạn để nhận mã khôi phục</p>
            </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-11 pr-4 py-3 text-sm rounded-xl border bg-slate-50 text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white transition-all ${
        ...
        {step === 2 && (
          <>
            <div className="text-center space-y-2">
              ...
              <h2 className="text-2xl font-bold text-slate-900 tracking-tight mt-3">Đặt Lại Mật Khẩu</h2>
              <p className="text-sm text-slate-500">Mã xác thực đã được gửi tới email <br/><span className="font-semibold text-slate-800">{email}</span></p>
            </div>
            ...
            <form onSubmit={handleStep2Submit} className="space-y-4">
              ...
              <div className="space-y-1.5 mt-4">
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider pl-1">Mật khẩu mới</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider pl-1">Xác nhận mật khẩu mới</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                    <Lock className="w-4 h-4" />
                  </div>
              ...
              <div className="text-center pt-2">
                <button
                  ...
                  disabled={sendOtpMutation.isPending || verifyOtpMutation.isPending || resetPasswordMutation.isPending}
        ...
        {step === 3 && (
          <div className="text-center py-6 space-y-6">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Thành công!</h2>
        ...
        {step === 1 && (
          <div className="text-center border-t border-slate-100 pt-4">
            <Link to="/login" className="text-sm font-semibold text-emerald-600 hover:text-emerald-700 inline-flex items-center gap-1">
              <ArrowLeft className="w-4 h-4" />
              Quay lại đăng nhập
            </Link>
          </div>
